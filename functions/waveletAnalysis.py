...
import sys
sys.path.append(r'C:\Users\hybth\Desktop\code\小波分析')
...
# READ THE DATA
# sst = np.loadtxt(r'C:\Users\hybth\Desktop\code\小波分析\sst_nino3.dat')  # input SST time series
sst = np.loadtxt(r'C:\Users\hybth\Desktop\code\小波分析\spei.txt')  # input SST time series
sst = sst - np.mean(sst)
...
n = len(sst)
dt = 1/12
time = np.arange(len(sst)) * dt + 2012  # construct time array
xlim = ([2012, 2021])  # plotting range
...
avg = np.logical_and(scale >=0.25, scale < 4)
...
plt.text(time[-1] + 1, 0.5, 'Wavelet Analysis\nC. Torrence & G.P. Compo\n'
